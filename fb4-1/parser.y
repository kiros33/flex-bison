...
opt_groupby: /* nil */
  | GROUP BY groupby_list
    opt_with_rollup       { emit("GROUPBYLIST %d %d", $3, $4); }
;
...
select_opts:                        { $$ = 0; }
  | select_opts ALL                 { if($1 & 0001) yyerror("duplicate ALL option");                  $$ = $1 | 0001; }
  | select_opts DISTINCT            { if($1 & 0002) yyerror("duplicate DISTINCT option");             $$ = $1 | 0002; }
  | select_opts DISTINCTROW         { if($1 & 0004) yyerror("duplicate DISTINCTROW option");          $$ = $1 | 0004; }
  | select_opts HIGH_PRIORITY       { if($1 & 0010) yyerror("duplicate HIGH_PRIORITY option");        $$ = $1 | 0010; }
  | select_opts STRAIGHT_JOIN       { if($1 & 0020) yyerror("duplicate STRAIGHT_JOIN option");        $$ = $1 | 0020; }
  | select_opts SQL_SMALL_RESULT    { if($1 & 0040) yyerror("duplicate SQL_SMALL_RESULT option");     $$ = $1 | 0040; }
...
join_table: table_reference opt_inner_cross JOIN table_factor opt_join_condition  { emit("JOIN %d", 100 + $2); }
  ...
  | table_reference left_or_right opt_outer JOIN table_factor join_condition      { emit("JOIN %d", 300 + $2 + $3); }
...
index_hint: USE KEY opt_for_join '(' index_list ')' { emit("INDEXHINT %d %d", $5, 10 + $3); }
  | IGNORE KEY opt_for_join '(' index_list ')'      { emit("INDEXHINT %d %d", $5, 20 + $3); }
  | FORCE KEY opt_for_join '(' index_list ')'       { emit("INDEXHINT %d %d", $5, 30 + $3); }
  | /* nil */
;
...
insert_stmt:
    ...
    VALUES insert_vals_list opt_ondupupdate         { emit("INSERTVALS %d %d %s", $2, $7, $4); free($4); } ;

opt_ondupupdate: /* nil */
  | ONDUPLICATE KEY UPDATE insert_asgn_list { emit("DUPUPDATE %d", $4); } ;

insert_opts: /* nil */        { $$ = 0; }
  | insert_opts LOW_PRIORITY  { $$ = $1 | 001; }
  | insert_opts DELAYED       { $$ = $1 | 002; }
  | insert_opts HIGH_PRIORITY { $$ = $1 | 004; }
...
insert_stmt:
    ...
    SET insert_asgn_list opt_ondupupdate { emit("INSERTASGN %d %d %s", $2, $6, $4); free($4); }
;
...
insert_stmt:
    INSERT insert_opts opt_into NAME opt_col_names
    select_stmt opt_ondupupdate                     { emit("INSERTSELECT %d %s", $2, $4); free($4); }
;
...
replace_stmt:
    ...
    VALUES insert_vals_list opt_ondupupdate         { emit("REPLACEVALS %d %d %s", $2, $7, $4); free($4); }
;

replace_stmt:
    REPLACE insert_opts opt_into NAME
    SET insert_asgn_list opt_ondupupdate            { emit("REPLACEASGN %d %d %s", $2, $6, $4); free($4); }
;

replace_stmt:
    REPLACE insert_opts opt_into NAME opt_col_names
    select_stmt opt_ondupupdate                     { emit("REPLACESELECT %d %s", $2, $4); free($4); }
;
...
create_table_stmt: CREATE opt_temporary TABLE opt_if_not_exists NAME create_select_statement { emit("CREATESELECT %d %d 0 %s", $2, $4, $5); free($5); };
